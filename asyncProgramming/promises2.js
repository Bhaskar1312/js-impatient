...
document.addEventListener('DOMContentLoaded', () => {
    {
        ...
        const promise1 = loadImage('1.png')
            .then(img => {
                imgdiv.append(img)
                return loadImage('2.png') // Another promise
            })
    ...
    }
    {
        ...
        const url = '1.png'
        loadImage(url)
            .then(img=> { // Promise is settled
                imgdiv.append(img)
            },
                reason => {// Promise is rejected
                    console.log({reason})
    ...
    }
    {
        ...
        loadImage(url)
            .then(img=> { // Promise is settled
                ...
                reason => {// A prior Promise is rejected
                console.log({reason})
                imgdiv.appendChild(brokenImage())
            }
            )
    ...
    { // Pipeline with a single catch
        const imgdiv = document.getElementById('images5')
        const url1 = '1.png';
        const url2 = '2.png';
        Promise.resolve()
            .then(()=> loadImage(url1))//  Asynchronous
            .then(img => imgdiv.appendChild(img)) //  Synchronous
            .then(()=> loadImage(url2))
            .then(img => imgdiv.appendChild(img))
            .finally(()=> console.log("Finally")/*doCleanUp()*/)
            .catch(reason => console.log({reason}))
    }
})

// concurrency - Promise.race(), Promise.any() Promise.all()
// If the iterable has non-promises, then one of them will be the result of the race.
// If the iterable is empty, then Promise.race(iterable) never settles.


//If the argument of the then method is not a function, the argument is discarded! The following is wrong:
// loadImage('1.png')
//   .then(img => imgdiv.appendChild(img))
//   .then(loadImage('2.png'))
